...
#ifndef __FPLS_SB_H__
#define __FPLS_SB_H__
...
class FPLS_SB : public FPLS_BASE<RegularizationType_, FPLS_SB<RegularizationType_>> {
   public:
    using RegularizationType = std::decay_t<RegularizationType_>;
    using Model = FPLS_SB<RegularizationType>;
    ...
    fdapde_enable_constructor_if(is_space_only, ModelBase)
      FPLS_SB(const pde_ptr& pde, Sampling s, RegularizedSVD<sequential> rsvd = RegularizedSVD<fdapde::sequential>{}) :
        ModelBase(pde, s, rsvd) {};
    fdapde_enable_constructor_if(is_space_time_separable, ModelBase)
      FPLS_SB(const pde_ptr& space_penalty, const pde_ptr& time_penalty, Sampling s,
    ...
    void regression(DMatrix<double>& X_h, DMatrix<double> & Y_h, std::size_t h, SmootherType smoother_, Calibrator<SmootherType> calibrator_) {
        // directions are used as loadings
        X_loadings_.col(h) = X_space_directions_.col(h);
    ...
    DMatrix<double> fitted(std::size_t h = 0) const {
        if(h == 0) h = n_comp();
        const DMatrix<double> & U_h = Y_latent_scores_.leftCols(h);
        const DMatrix<double> & S_h = Y_loadings_.leftCols(h);
        return U_h * S_h.transpose();
    }
