...
class FPLS_A : public FPLS_BASE<RegularizationType_, FPLS_A<RegularizationType_>> {
   public:
    using RegularizationType = std::decay_t<RegularizationType_>;
    using Model = FPLS_A<RegularizationType>;
    ...
    fdapde_enable_constructor_if(is_space_only, ModelBase)
      FPLS_A(const pde_ptr& pde, Sampling s, RegularizedSVD<sequential> rsvd = RegularizedSVD<fdapde::sequential>{}) :
        ModelBase(pde, s, rsvd) {};
    fdapde_enable_constructor_if(is_space_time_separable, ModelBase)
      FPLS_A(const pde_ptr& space_penalty, const pde_ptr& time_penalty, Sampling s,
    ...
    void regression(DMatrix<double>& X_h, DMatrix<double> & Y_h, std::size_t h, SmootherType smoother_, Calibrator<SmootherType> calibrator_) {
        ...
        Y_loadings_.col(h) = Y_h.transpose() * Y_latent_scores_.col(h) / Y_latent_scores_.col(h).squaredNorm();
        return;
    }
